...
function select_direction(element) {
    ...
    if(element.life.food > (200 * genome_decode(element, 1)) && element.possibility.includes('d') && document.getElementById(element.directions['d']).classList.contains('white') && how_many_alive_cells() < max_alive_cells) {
    ...
    if(element.food_around.length == 0) {
        ...
        if(HereOrNo > 0.5 / genome_decode(element, 4)) {
        ...
        for(i = 0; i < element.food_around.length; i++) {
            if(!document.getElementById(element.directions[element.food_around[i]]).classList.contains('target')) {
                direction = element.food_around[i]
                return direction
            }
        }
        HereOrNo = Math.random()
